...
def main(config):
    ...
    if rep.status_code != 200:
        fail("TeslaFi request failed with status %d", rep.status_code)
    # print(rep.json()['display_name'])
    # print(rep.json()['battery_range'])
    # print(rep.json()['usable_battery_level'])
    # print(rep.json()['charge_limit_soc'])
    # print(rep.json()['battery_level'])
    # print(rep.json()['est_battery_range'])
    
    name=rep.json()['display_name']
    rangemi=rep.json()['est_battery_range']
    if(config.bool("mi2km")==True):
        rangemi=math.round((float(rangemi)*1.60934)*100)/100
    # battlevel=rep.json()['battery_level']
    chargelimit=rep.json()['charge_limit_soc']
    batterylevel=rep.json()['usable_battery_level']
    chargingstate=rep.json()['charging_state']
